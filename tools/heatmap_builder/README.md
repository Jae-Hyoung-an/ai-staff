...
## 입력 형식 (CSV / XLSX)
CSV(인코딩 `cp949`/`utf-8` 자동 인식)와 XLSX(첫 시트) 모두 지원하며, 여러 파일을 동시에 병합합니다. 컬럼명은 아래 별칭을 자동 인식합니다.

| 항목 | 허용 컬럼명(별칭) | 예시 |
|------|------------------|------|
| 위도 | `위도` | 37.6131654 |
| 경도 | `경도` | 127.0767934 |
| 상호명 | `상호명` / `상점명` / `사이트_상호명` | 금용각 |
| 가게배달유무 | `가게배달유무` / `가게배달여부` | Y / N / 기존오더 |
| 주문수 | `최근주문수` / `오더수` | 45000 |
| 가게배달 가격(선택) | `가게배달가격` / `가게배달 가격` | 3,000원 |

> `가게배달유무` 값 `기존오더`는 별도 카테고리로 처리되어 "기존오더만" 필터로 볼 수 있습니다.
> `가게배달 가격`은 있는 자료만 표시되고, 없으면 공란으로 나옵니다.
