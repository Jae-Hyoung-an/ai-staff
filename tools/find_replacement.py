...
current_a = {48734, 253736, 255821, 252667, 95551}
anchor = (37.593551, 127.075380)

# Other Option A stores (excluding 252667)
other_stores = [
...
print("=== 252667 대체 후보 (2건 이상, 기준점 2km 이내) ===")
print(f"{'store_id':>8} | {'동':>6} | {'건수':>4} | {'기준점거리':>8} | 11시 | 12시 | 13시 | 기존상점과 최대거리")
print("-" * 95)

candidates = []
for e in rows:
    if e['store_id'] in current_a and not (e['store_id'] == 252667):
        continue
    if e['store_id'] == 252667:
        continue
    if e['total'] < 2:
        continue
...
print(f"\n참고: 제외 대상 store 252667 (면목동, 4건, 기준점 1.25km)")
