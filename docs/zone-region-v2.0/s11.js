...
S.ven = {
  ...
  notes:[
    {t:"검색 조건 — 라벨이 바뀌고 둘이 는다", d:"<span class='strike'>소속 Zone</span> → <b>배송권역</b>으로 라벨과 대상이 바뀐다. 여기에 <b>정책명</b>(권역을 거쳐 파생된 값)과 <b>연결 상태</b>(권역 연결 / 권역 미연결)가 는다. 벤더명 · 상태는 그대로다.", r:"11 §4.2"},
    {t:"권역 미연결 N건 — 이 화면에만 있다", d:"<b>세트 분배 목록에서는 찾을 수 없는 정보다.</b> 그 화면은 권역 기준이라 어느 권역에도 없는 벤더는 어느 행에도 나오지 않는다. 등록만 해두고 붙이는 걸 잊으면 <b>계약은 되었는데 물량이 안 가는</b> 상황이 되는데, 그걸 잡는 장치는 여기뿐이다.", r:"11 §4.2 · S-2"},
    {t:"소속 배송권역 (컬럼 변경 · 링크)", d:"<span class='strike'>존</span> 자리다. 연결이 없으면 <b>미연결</b>을 강조해 보여준다. 누르면 <b>그 권역을 담고 있는 정책 상세</b>로 이동한다(새 탭).", r:"11 §4.2 · VEN-1"},
    ...
    {t:"소속 기사 수 (신설 · 링크)", d:"이 벤더에 속한 기사 수. 누르면 <b>소속기사 탭</b>으로 간다. <b>권역 미연결 벤더에서는 그 수만큼 오더를 못 받고 있다는 뜻</b>이라 미연결 목록의 처리 순서를 정하는 기준이 된다. <b>QA2 목록에는 이 컬럼이 없다</b> — 개발과 맞춰야 할 차이다.", r:"11 §4.2 · VEN-7"},
  ...
  + plain({title:"벤더",axis:"cost"},
      ...
      tbl([{t:"벤더 코드",num:1},{t:"사업자명(벤더명)"},{t:"상태"},{t:"소속 배송권역",mk:3},{t:"정책명",mk:4},{t:"세트 수",num:1,mk:5},{t:"소속 기사 수",num:1,mk:6},{t:"벤더장"},{t:"등록일",mono:1}],[
        ["14",'<span class="lnk" data-go="ven2">부릉 강남 벤더</span>',V.on,ext("강남 벤더 배송권역 A"),ext("수도권 벤더 운영정책 v3"),'<span class="lnk">5개</span>','<span class="lnk" data-go="ven3">24명</span>',"A벤더장","2026-09-01"],
...
S.ven2 = {
  ...
  notes:[
    {t:"이 문서의 유일한 실질 변경", d:"사업자 · 정산 · VO · 첨부 파일 · 소속기사 탭은 <b>그대로 둔다.</b> 바뀌는 것은 <b>소속의 축 하나</b> — 벤더가 속하는 대상이 존에서 벤더 배송권역으로 바뀌고, 그 결과 계약 정보에서 한 칸이 빠지고 섹션 하나가 는다.", r:"11 §1 · §4.3"},
    {t:"계약 정보에서 소속 Zone을 뺀다", d:"<span class='strike'>소속 Zone</span> 입력란은 <b>Spec-out</b>이다. 소속을 정하는 자리가 세트 분배로 옮겨졌으므로 이 화면에는 고르는 칸이 없다. <span class='kbd'>상태</span>도 아래 소속 권역 섹션으로 옮긴다.", r:"11 §4.3 섹션3 · VEN-3"},
    {t:"소속 권역 (신설 · 조회 전용)", d:"<b>소속 배송권역 · 연동 지점 · 속한 정책 · 적용 원가 요금제 · 이 권역에서 받는 세트 수 · 상태</b> 여섯을 보여준다. 배송권역 · 정책 · 요금제 · 세트 수는 각각 해당 상세로 이동한다(새 탭).", r:"11 §4.3 섹션4"},
    {t:"바꾸는 자리는 여기가 아니다", d:"입력란을 두지 않고 <span class='kbd'>세트 분배에서 변경</span> 버튼으로 <b>세트 분배의 해당 권역 상세</b>로 보낸다. 소속을 바꾸는 저장은 기사 소속 지점까지 옮기므로 그 화면에서만 일어난다 (10 SET-4).", r:"11 §4.3 섹션4 · VEN-3"},
    {t:"연결이 없으면 배너를 띄운다", d:"<span class='kbd'>이 벤더는 어느 배송권역에도 연결되어 있지 않아, 소속 기사가 오더를 받지 못합니다.</span> 등록은 됐지만 <b>기사가 지점에 배치된 채 오더를 못 받는 상태</b>임을 이 자리에서 알린다.", r:"11 §4.3 섹션4 · VEN-7"},
    {t:"원가 금액은 적지 않는다", d:"<b>요금제명만</b> 보여준다. 금액은 요금 상품 관리에서 본다. 벤더가 「우리는 어떤 조건이냐」고 물어오면 여기서 요금제명을 눌러 따라간다 (S-3).", r:"11 §4.3 섹션4 · S-3"},
    ...
    {t:"해지는 되돌릴 수 없다", d:"<span class='kbd'>대기 → 운영중 ↔ 운영중지 → 해지</span>. 권역에 연결된 벤더나 소속 기사가 있는 벤더를 해지하려 하면 <b>먼저 경고 모달</b>이 뜬다 (SCR_VEN_004).", r:"11 VEN-8 · §4.5"}
  ],
  render(){ return head({
      h:"강남 라이더스",
  ...
  + `<div${note(1)} style="margin-top:14px">${banner("ok","그대로", "사업자 정보 · 정산 정보 · 벤더장(VO) · 첨부 파일 · 소속기사 탭은 <b>이번 개편에서 바뀌지 않는다.</b> 바뀌는 것은 <b>소속의 축 하나</b> — 계약 정보에서 「소속 Zone」이 빠지고, 「소속 권역」 섹션이 는다." + mk(1))}</div>`
  + `</div>`
  ...
  + card({title:"계약 정보",tag:"변경",tagk:"warn",mk:2}, form([
      ...
      {l:"소속 Zone",html:'<div class="field ro"><span class="strike">부릉 강남2존</span> &nbsp;<span class="pill stop">Spec-out</span></div>',hint:"<b>입력란을 없앤다.</b> 소속을 정하는 자리는 세트 분배 관리다 (VEN-3)"},
      {l:"상태",html:'<div class="field ro"><span class="strike">운영중</span> &nbsp;<span class="pill warn">아래 소속 권역 섹션으로 이동</span></div>'}
    ]))
  + `</div>`
  ...
  + card({title:"소속 권역",tag:"신설 · 조회 전용",tagk:"new",mk:3,axis:"cost"},
      ...
      + form([
        ...
        {l:"연동 지점",ro:1,html:'<div class="field ro">'+ext("부릉 서초지점")+'</div>',hint:"이 벤더 소속 기사의 지점이 여기로 동기화된다 (VEN-6)"},
        ...
        {l:"상태",ro:1,html:'<div class="field ro">'+V.on+'</div>',hint:"계약 정보에서 이 자리로 옮겨왔다"}
      ])
  ...
  + card({title:"벤더장 (VO) 지정 / 해제",tag:"그대로",tagk:"mute",mk:7}, form([
    ...
    + `<div class="hint g" style="margin-top:10px">» 벤더 기사로 등록 가능한 기사 중에서만 검색됩니다 (이름 또는 연락처). <b>타 벤더 VO 기사는 서버에서 선택이 제한됩니다.</b></div>
       ...
       <div class="hint">위 두 줄은 <b>구현이 이미 지키고 있는 규칙</b>인데 문서에 없었다 — PRD §4.3 저장 검사표에 명문화했다.</div>`)
  ...
  + plain({title:"저장할 때 검사하는 것"}, tbl([{t:"#",mono:1},{t:"상황"},{t:"화면이 하는 일"}],[
      ...
      ["5","운영중 상태에서 VO 해제","<b>막는다.</b> 교체만 가능하다 — 해제하려면 먼저 운영중지로 바꾼다"],
      ["6","타 벤더의 VO 기사를 벤더장으로 선택","<b>후보에 나오지 않는다.</b> 서버에서 제한한다"]
    ]))
  + bar([{t:"운영 중지",off:1},{t:"해지",off:1},{t:"목록으로"},{t:"저장",pri:1}])
  + `<div class="foot"${note(8)}><span><b>탭</b> 기본정보 / 소속기사 / 변경 이력</span><span class="strike">계약 정보의 소속 Zone</span><span><b>신설</b> 소속 권역 섹션 · 세트 분배에서 변경</span><span><b>등록 화면</b> 소속 칸 없음 — 등록 후 「세트 분배에서 연결하기」 안내</span>${mk(8)}</div>`;}
};
...
S.ven3 = {
  ...
  notes:[
    ...
    {t:"소속 지점은 여기서 고치는 값이 아니다", d:"기사의 소속 지점은 <b>소속 벤더가 속한 벤더 배송권역의 지점</b>으로 자동 동기화된다. 벤더가 권역을 옮기면 따라 움직이고, 권역과 분리되면 기존 지점을 유지한다. <b>컬럼으로 두지는 않는다</b> — 소속 지점은 기사 현황 화면에서 본다.", r:"11 VEN-6 · §4.4"},
    {t:"동기화가 일어나는 네 경우", d:"① 기사가 <b>소속 벤더를 바꾸면</b> 새 벤더의 권역 지점으로 자동 이동 ② 벤더가 <b>권역을 옮기면</b> 따라 이동 ③ 벤더가 <b>권역과 분리되면</b> 기존 지점 유지 ④ 벤더가 <b>처음부터 미연결이면</b> 운영이 지정한 지점에 수동 배치. <b>파생값이 아니라 저장값인 이유가 ③·④</b>다.", r:"11 VEN-6"},
    {t:"타 벤더 소속 기사는 등록되지 않는다", d:"<b>벤더 기사로 등록 가능한 기사 중에서만</b> 검색된다. 이미 다른 벤더에 소속된 기사는 서버에서 등록이 제한된다.", r:"11 §4.4 · QA2 구현"},
    {t:"벤더가 미연결이면", d:"기사들은 운영이 지정한 지점에 배치된 채로 남고 <b>오더는 받지 못한다.</b> 지점별 기사 수 통계에는 잡히고 실적에는 안 잡힌다. <b>수동 배치를 어느 화면에서 하는지는 미결</b>이다 — 지금은 어느 화면에도 없다 (미결 ①).", r:"11 VEN-7 · §6 미결 ①"}
  ],
  render(){ return head({
      h:"강남 라이더스 — 소속기사",
  ...
  + plain({title:"기사 소속 지점이 움직이는 네 경우",mk:3,axis:"cost"},
      tbl([{t:"#",mono:1},{t:"무슨 일이 있었나"},{t:"소속 지점"}],[
        ["①","기사가 <b>소속 벤더를 바꿨다</b>","새 벤더가 속한 권역의 지점으로 <b>자동 이동</b>"],
        ["②","벤더가 <b>다른 권역으로 옮겨졌다</b> (10 세트 분배 저장)","<b>따라 이동</b> — 저장 시 ① 영향 고지 모달이 이 인원을 보여준다"],
        ["③","벤더가 <b>권역과 분리됐다</b> (세트 분배에서 삭제 · 09에서 권역 해제)","<b>기존 지점을 유지</b>한다"],
        ["④","벤더가 <b>처음부터 미연결</b>이다","운영이 지정한 지점에 <b>수동 배치</b> — <span class='pill warn'>미결 ①</span> 어느 화면에서 하는지 미정"]
      ]))
  + `<div class="foot"${note(5)}><span><b>이 탭에서 고치는 값이 아니다</b> — 소속 지점은 벤더의 권역을 따라간다</span><span><b>미결 ①</b> 미연결 벤더 기사의 수동 배치 입구</span><span><b>미결 ⑤</b> 기사 소속 지점 이동 이력</span>${mk(5)}</div>`;}
};
...
S.ven4 = {
  ...
  notes:[
    ...
    {t:"권역 연결은 자동으로 풀리지 않는다", d:"<span class='kbd'>이 벤더는 {배송권역명}에 연결되어 있습니다. 해지하면 세트 분배에서 연결을 해제해야 합니다.</span> — <b>해지가 연결을 대신 끊어주지 않는다.</b> 세트 분배로 가서 행을 빼고 저장해야 한다.", r:"11 §4.3 저장 검사 1 · §4.5"},
    {t:"기사는 수를 보여주고 확인만 받는다", d:"기사 소속 처리는 <b>기존 절차를 따른다</b> — 이 모달이 기사를 옮기지 않는다. 몇 명이 걸려 있는지만 알리고, <b>소속 지점은 그대로 유지된다</b>는 것을 함께 적는다 (VEN-6 ③).", r:"11 §4.3 저장 검사 2 · VEN-6"},
  ...
  render(){ return head({
      h:"해지 경고 모달",
      p:"<span class='kbd'>해지</span>를 누른 직후 뜬다. <b>권역 연결</b>과 <b>소속 기사</b> 두 가지를 한 화면에 모아 보여주고 확인을 받는다.",
      owner:"공급 담당자", review:"벤더 서버 담당 팀"})
  ...
  + plain({title:"연결된 벤더를 해지한 뒤에 할 일"}, tbl([{t:"순서",mono:1},{t:"어디서"},{t:"무엇을"}],[
      ...
      ["2","저장 확인 모달","<b>기사 N명은 소속 지점을 그대로 유지합니다</b>로 뜬다 (09 POL-13)"],
      ["3","11. 벤더 관리","이 벤더는 <span class='dash hot'>미연결</span> · <span class='pill stop'>해지</span> 상태로 목록에 남는다 — <b>벤더는 삭제하지 않는다</b>"]
    ]))
  + `<div class="foot"${note(5)}><span><b>액션</b> 취소 · 해지 — 막는 모달이 아니라 정리 순서를 알려주는 모달이다</span><span><b>벤더는 삭제하지 않는다</b> 상태 전이로만 다룬다 (VEN-8)</span>${mk(5)}</div>`;}
};
