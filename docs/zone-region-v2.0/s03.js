...
S.pt = {
  ...
  + plain({title:"영업존 권역",tag:"신설",tagk:"new",axis:"sales",mk:4}, `<div class="body">
      <div class="gridsplit">
        <div>
          ...
          ${tbl([{t:"영업존 권역명"},{t:"연결 상태",mk:6},{t:""}],[
            ["<b>부릉 강남2지점 영업존 권역</b>",ext("강남 영업존"),'<button class="btn sm">편집</button>']
          ])}
  ...
  + plain({title:"벤더 배송권역",tag:"신설",tagk:"new",axis:"cost",mk:7}, `<div class="body">
      ...
      <div class="gridsplit">
        <div>
          ${tbl([{t:"벤더 배송권역명"},{t:"속한 정책"},{t:""}],[
            ["<b>강남 벤더 배송권역 A</b>",ext("수도권 벤더 운영정책 v3"),'<button class="btn sm">편집</button>']
          ])}
  ...
  + plain({title:"권역이 없을 때",tag:"참고",tagk:"mute",mk:5}, `<div class="body">
      ...
      <div class="hint">기준 화면의 <span class="kbd">등록된 권역이 없습니다.</span> 자리다. <b>왜 그려야 하는지를 함께 적는다</b> — 권역이 없는 지점은 02·09의 지점 선택 목록에서 감췄지기 때문이다 (02 EZ-4).</div>
    </div>`)
  + `</div>`
  ...
  + plain({title:"저장할 때 검사하는 것",mk:9}, tbl([{t:"#",mono:1},{t:"상황"},{t:"화면이 하는 일"}],[
      ...
      ["6","내가 편집하는 사이 <b>다른 사람이 04에서</b> 같은 권역을 고쳤 저장","<b>충돌로 보고 저장을 막는다.</b> 내 초안으로 덮어쓰지 않는다 (SCR_PT_004)"],
      ["7","운영방식을 바꾸려 함 — <b>쓰는 곳이 없을 때</b>","<b>그냥 바뀜다.</b> 탭이 사라지고 경고도 없다. 그려 둔 권역은 <b>보관</b>되어 있다가 다시 「벤더 존」으로 되돌리면 그대로 나온다"],
...
S.pt2 = {
  ...
  notes:[
    {t:"영업존 권역에서만 뜬다", d:"벤더 배송권역 섹션에서는 <b>어떤 경우에도 뜬지 않는다.</b> 겹침이 오류인 축과 의도인 축이 갈라져 있다.", r:"03 §4.6 · PT-5"},
  ...
  render(){ return head({
      h:"중첩 안내 팝업",
      p:"<b>영업존 권역 섹션</b>에서 저장을 누르고 다른 영업존의 권역과 겹쳤을 때 뜬다. 벤더 배송권역에서는 뜬지 않는다.",
      owner:"권역 담당자", review:"권역 서버 담당 팀"})
  + modal({title:"저장할 수 없습니다",tag:"신설",tagk:"stop",actions:[{t:"확인",pri:1}]},
      `<div class="hint g" style="margin:0 0 12px"${note(2)}>» 이 권역이 아래 영업존의 권역과 겹칩니다. 겹치는 구간을 없앱 뒤에 저장해주세요.${mk(2)}</div>
...
S.pt3 = {
  ...
  notes:[
    ...
    {t:"쓰는 곳이 없으면 그냥 바뀜다", d:"<b>경고하지 않는다.</b> 탭이 사라지고 끝이다. 그려 둔 권역은 <b>지워지지 않고 보관되며</b>, 다시 <span class='kbd'>벤더 존</span>으로 되돌리면 그대로 나온다.", r:"03 PT-11 · §4.7 7번"},
  ...
  + card({title:"경우에 따라 달라진다",mk:2}, tbl([{t:"쓰는 곳"},{t:"운영방식 변경"},{t:"그려 둔 권역"}],[
      ["<b>없다</b>",'<span class="pill ok">그냥 바뀜다</span> — 경고도 하지 않는다',"<b>지워지지 않고 보관된다.</b> 다시 「벤더 존」으로 되돌리면 그대로 나온다"],
...
S.pt4 = {
  ...
  notes:[
    ...
    {t:"누가 언제 고쳬는지를 적는다", d:"상대의 <b>이름 · 저장 시각 · 저장 사유</b>를 보여준다. 사유가 필수이므로(PT-7) 왜 고쳬는지가 항상 남아 있다 — 다시 그리기 전에 읽어볼 수 있다.", r:"03 PT-7 · §4.7 6번"},
  ...
  render(){ return head({
      h:"편집 충돌 팝업",
      p:"내가 편집하는 사이 <b>다른 사람이 04 지점 권역 관리에서</b> 같은 권역을 고쳤 저장했을 때 뜬다.",
      owner:"권역 담당자", review:"권역 서버 담당 팀"})
