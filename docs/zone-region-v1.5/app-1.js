...
S.ez = {
  ...
  notes:[
    ...
    {t:"판가 요금제 컴럼", d:"요금제 쪽 목록의 대상 존 수가 0으로 보이는 결함 때문에 존 쪽에서 거꿒로 보여준다. 연결이 없으면 강조한다.", r:"02 §5.1"},
...
S.ez2 = {
  ...
  + plain({title:"이력",tag:"신규",mk:6,actions:[{t:"삭제"},{t:"목록으로"},{t:"저장",pri:1}]},
      tbl([{t:"일시",mono:1},{t:"변경한 사람"},{t:"바뀜 것"}],[
...
S.vdr = {
  ...
  notes:[
    {t:"독립 메뉴로 승격", d:"정책 화면 안의 탭이던 것을 독립 메뉴로 뻔다. 이 권역이 원가·정산·세트분배·벤더 소속의 기준 축이다.", r:"03 §2"},
    {t:"연동 지점은 항상 값이 있다", d:"지점 연동 없는 배송권역은 만들 수 없다. 지점을 고른 뒤 그 지점의 공급권역 중 하나를 고른다.", r:"03 VDR-1 · VDR-5"},
    {t:"공급권역명은 별도 컴럼", d:"운영자가 붙인 배송권역명과 원본 공급권역명이 다를 수 있어 둘 다 보여준다.", r:"03 §5.1"},
    {t:"끊긴 연결을 걸러 본다", d:"정책이나 벤더가 붙지 않은 권역은 배차에 쓰이지 않는데 화면에는 정상으로 보인다. 상단 칩으로 그 행만 걸러난다.", r:"03 §5.1"},
  ...
  + `<div class="foot"><span><b>한 지점 여러 권역</b> 성북 1 지점이 두 권역을 갖는 것은 정상이다 (지점 → 배송권역 = 1:N)</span><span class="strike">중첩 정책 컴럼</span></div>`;}
};
...
S.vdr2 = {
  ...
  + plain({title:"이력",tag:"신규",actions:[{t:"삭제"},{t:"목록으로"},{t:"저장",pri:1}]},
      tbl([{t:"일시",mono:1},{t:"바뀜 것"},{t:"왜 남기나"}],[
        ["2026-08-19 11:20","연동 공급권역 모양 변경 (12.0 → 12.4 km²)","원가 산정 범위가 바뀜 것이므로 정산 분쟁 때 언제부터 달랐는지를 봐야 한다"],
