...
flowchart TB
    ...
    subgraph Step2["Step 2: 권역 기획 — L-4주~L-3주"]
        direction TB
    ...
    subgraph Step3["Step 3: 원/판가 + 영업기준 — L-3주~L-2.5주"]
        direction TB
        S3A["영업: 판가 설정"]
        S3B["운영: 원가 요금제 + 보상기준 설계"]
        S3D["영업+운영: Zone별 영업 기준표 작성"]
        S3E["기획+영업+운영: 영업 개시 일자 협의"]
        S3C["기획: P&L 검증 후 승인"]
        S3A --> S3C
        S3B --> S3C
        S3D --> S3C
    ...
    subgraph parallel["Step 4-1 / 4-2 병렬 — L-2.5주~L-1주"]
        direction LR
    ...
    Step3 -->|"원/판가 확정"| Step4a
    Step3 -->|"원/판가 확정"| Step4b
...
| **Zone별 영업 기준표** | Zone별 운영시간, 배송 권역 등 영업 기준 정보 | 영업+운영 공동 작성 → 기획 승인 |
| **영업 개시 일자** | 해당 지역 서비스 런칭(영업 개시) 일자 확정 | 기획+영업+운영 협의 후 기획 확정 |
...
| 영업 기준표 확정 | Zone별 운영시간, 배송 권역 포함 영업 기준표 기획조직 승인 |
| 영업 개시 일자 확정 | 3조직 협의 완료, 기획조직 최종 확정 |
...
데이터조직이 과거 물량 트렌드 및 프렌즈 생산성 분석 자료를 제공하고, 기획조직이 이를 참고하여 **Zone별·주차별 수행 가능 세트 수 및 활성 프렌즈 수 초안을 하달**하며, 운영조직이 공급 실현성 관점에서 기획조직과 협의하여 최종 확정한다.

```
데이터: 과거 물량 트렌드 분석 + 세트 물량 단위 구성 + 프렌즈 생산성 산정 → 기획에 참고 자료 제공
  ↓
기획: 데이터 자료 참고하여 Zone별·주차별 수행 가능 세트 수 + 활성 프렌즈 수 초안 하달
  ↓
...
| 활동 | 설명 |
|------|------|
| 과거 물량 트렌드 분석 | 유사 지역의 과거 오더 발생 패턴, 요일/시간대별 물량 추이를 분석하여 기획에 제공 |
| 세트 물량 단위 구성 | 과거 트렌드를 참고하여 세트당 적정 물량 단위를 산정하고 기획에 제공 |
| 프렌즈 생산성 산정 | 프렌즈 인당 시간대별 수행 가능 건수, 가동률 등 생산성 지표를 산정하여 기획에 제공 |
