...
> 작성자: @jaehyoung.an
> 작성일: 2026-07-13 / 최종 수정: 2026-07-29
...
현행 ZONE 요금제는 **하나의 ZONE이 판가와 원가를 동시에 소유**하고, ZONE이 **영업 권역(픽업)과 공급 권역(배송)을 1:1로 강결합**한 구조다. 권역 간 중첩도 불가하다. 이 때문에 다음 문제가 상시 발생한다.
...
| 3 | **배송권역 정책 개편** | `적용 ZONE 선택` → `벤더 배송권역`(중첩 허용) 대체. `원가 요금제` 탭·`원가 할증` 탭 신설 |
...
| **픽업지 권역** | 지점이 보유한 픽업 폴리곤(`PICK_UP_REGION`). 지점당 N개. 하나의 픽업지 권역은 **하나의 영업존에만** 연결된다 | - |
...
| 5.1.2 | **카디널리티** | 픽업지 권역 : 영업존 = **1:1**. 지점 : 영업존 = **1:N** (지점이 픽업지 권역을 여러 개 보유하면 각각 별도 영업존을 만들 수 있다) |
...
| 5.2.2 | **매핑 카디널리티** | 영업존당 판가 요금제 1개. 판가 요금제는 복수 영업존에 적용 가능 (현행 규칙 유지) |
...
| 5.3.2 | **원가 할증** | **`원가 할증 관리` 메뉴를 신설**한다(판가 할증과 같은 층위). 여기서 생성·관리하고, 배송권역 정책의 원가 요금제 탭에서 **적용 할증을 선택**한다. 원가 할증 : 배송권역 정책 = N:M |
...
| 5.4.3 | **공급권역 처리** | 공급권역의 **영업기준(도착권역) 역할은 폐기**하고 **지오메트리 소스로만 재활용**한다. 상점 배송 가능 범위 판정에서는 분리된다(5.1.6) |
| 5.4.4 | **정책 연결** | 배송권역 : 배송권역 정책 = N:1. 하나의 정책이 여러 지점의 공급권역을 참조할 수 있다 |
...
| **라이더비(기본 원가)** | **요일 × 슬롯 매트릭스** | 행 = 요일(7), 열 = 활성 슬롯(최대 5) + **시간 외** 1열. 각 셀에 기본 거리 구간의 라이더비 입력 |
...
- 어떤 배송권역에도 속하지 않는 오더는 폴백 원가를 적용한다 (TBD-8).
...
| 5.8.3 | **구버전 대응** | 앱 클라이언트가 `agentFee` + `extraChargeFee`를 합산 표시하는 현행 구조상, 태그 노출은 앱 배포에 종속된다. 구버전은 **금액만 정확히 표시되고 태그는 미노출**을 허용한다 |
...
| TBD-2 | **기사 후보 공급 API 변경** | 현행 `/api/v2/zones/{zoneUuid}/dispatch-candidates`(존 단위)를 권역 기준으로 어떻게 바꿀지 | 5.6.1 |
| TBD-3 | **벤더:배송권역 정책 카디널리티** | 현행 벤더:존 = N:1. 중첩 허용 시 한 벤더가 복수 정책에 속할 수 있는가 | 5.4.2 |
...
| TBD-7 | **원가 이력 전파 범위** | `pricingPolicyUuid`가 정산·스트림으로 전파되지 않음. 감사 요건이 있으면 신규 배관 필요 | 5.6.2 |
| TBD-8 | **폴백 원가 정의** | 어떤 배송권역에도 속하지 않는 오더의 원가 | 5.6.3 |
| TBD-9 | **지점 : 영업존 1:N의 운영 영향** | 한 지점이 채널별로 여러 영업존을 갖게 되면 지점 단위 리포팅·정산 집계에 영향이 있는지 | 5.1.2 |
...
- 영업존을 **픽업지 권역 1개**로 확정하면서 지점 : 영업존이 1:N이 됐다. 채널별 판가 확장 경로가 열리는 대신 **지점 단위 집계에 영향이 있을 수 있어** TBD-9로 남겼다.
