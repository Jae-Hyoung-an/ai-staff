...
| 2026-07-30 | @jaehyoung.an | **v1.3** — 기획 문서화 정리: 화면 URL·백엔드 요구사항·필드 타입 등 구현 상세 제거(개발팀이 별도 작성), 기술 용어 일반화(카디널리티·폴백 등), 동작·결과 중심 재서술 |
| 2026-07-30 | @jaehyoung.an | **v1.3 정책 보강** — 영업존 픽업지 권역 지리 중첩 시 저장 차단, 상점 최대 배송 반경은 배송 반경값만 반영(픽업지 권역 미반영), 원가 할증 메뉴를 세트 관리 하위로 이동, 관리비 150원 정산 근거 문구 삭제. 문서 순서: PRD → 유저 시나리오 → 기능 명세 |
...
| # | 항목 | 요약 |
|---|------|------|
| 1 | **영업존 관리 개편** | 지점 + 픽업지 권역 1개로 영업존 구성, 픽업지 권역 중복 연결·지리 중첩 저장 차단, 상점 최대 배송 반경은 배송 반경값만 반영 |
...
| **상점 판매권역** | 상점이 배송을 요청할 수 있는 범위(`STORE_SALES_REGION`). **배송 반경값**으로 상점의 최대 배송 가능 반경이 결정되며, 영업존의 픽업지 권역은 반영하지 않는다 | - |
...
| (없음) | 세트 관리 > **원가 할증 관리** (신설) |
| 세트 관리 > ZONE 권역정책 관리 | 세트 관리 > **배송권역 정책 관리** |
| (없음) | **권역 시각화** (신설) |

- 판가 요금제·판가 할증은 **목록 화면부터 진입**하도록 통일한다. AS-IS에서 목록 없이 상세로 직행하던 동선을 정리한다. 원가 할증도 목록 우선 진입이다.
...
| 5.1.4 | **지리적 중첩** | 영업존에 연결된 픽업지 권역의 폴리곤이 **다른 영업존의 픽업지 권역과 지리적으로 겹치면 저장 불가**. 신규 영업존 등록·연결된 픽업지 권역 수정 시 검증한다. 같은 지점의 채널별 픽업지라도 폴리곤이 겹치면 차단한다 |
| 5.1.5 | **판가 적용** | 영업존에 연결된 픽업지 권역 소속 **G4(직영센터) 상점은 오더 접수 시 해당 판가·판가 할증·프로모션을 자동 적용**받는다. 판정 기준은 **관제지점 소속**이며, 예외 상황용 수동 오버라이드를 유지 |
| 5.1.6 | **상점 판매권역** | **배송 반경(영업존 단일값)** 으로 상점의 최대 배송 가능 반경이 결정된다. **영업존의 픽업지 권역은 반영하지 않는다.** 벤더 배송권역과도 무관 |
| 5.1.7 | **거리별 판가** | 기존 구조 유지 (구간 반복 + 정액제/거리별) |

**왜 픽업지 권역 1개인가**: 지점 세일즈 권역은 픽업지 권역의 합집합이라 부분 선택이 불가능하다. 픽업지 권역 단위로 쪼개면 영업존 하나가 픽업지 하나를 갖게 되므로 "이 상점의 판가는 무엇인가"가 한 단계로 결정된다.

> ⚠ **채널별 판가 제약**: 지리 중첩을 전면 차단하므로, 성북처럼 채널별 픽업지(`YOGIYO_OD`, `YOGIYO_DIRECT` 등)가 **동일 폴리곤**인 경우 채널별 영업존을 동시에 등록할 수 없다. 채널별 판가를 쓰려면 **폴리곤이 서로 겹치지 않도록 권역을 먼저 분할**해야 한다.
...
| 5.3.2 | **원가 할증** | **`세트 관리 > 원가 할증 관리` 메뉴를 신설**한다. 여기서 생성·관리하고, 배송권역 정책의 원가 요금제 탭에서 **적용 할증을 선택**한다. 하나의 할증을 여러 정책에 적용할 수 있고, 한 정책에 여러 할증을 붙일 수 있다 |
...
- [02_유저 시나리오](./02_유저_시나리오.md) — 시나리오 및 엣지케이스
- [03_기능명세](./03_기능명세.md) — 화면·필드 단위 상세
...
- 본 PRD는 **리뷰 버전(v1.3)** 이다. 이번 개편에서 무엇을 만들 것인지에 집중하고, 기존 정책 문서 개정·타 개발 건과의 병합은 [R1](./references/R1_기존정책_크로스체크_및_충돌.md)에 분리해 **별건으로 진행**한다.
- 영업존을 **픽업지 권역 1개**로 확정하면서 한 지점이 여러 영업존을 가질 수 있게 됐다. 다만 **지리 중첩 저장 차단**으로 동일 폴리곤 채널 픽업지는 동시에 영업존으로 등록할 수 없다. 채널 분리는 권역 분할이 선행돼야 하며, 지점 단위 집계 영향은 TBD-9로 남겼다.
- 상점 최대 배송 가능 반경은 **배송 반경값만**으로 결정하고, 영업존 픽업지 권역은 반영하지 않는다.
- 리뷰에서 결정할 것은 **TBD-1 ~ TBD-9**이며, 이 중 TBD-1(정산 집계 축)·TBD-2(기사 후보 조회 방식)는 vendorset·dispatch 팀이 참석해야 결론이 난다.
